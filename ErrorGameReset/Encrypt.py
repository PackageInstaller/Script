...
if __name__ == '__main__':
    ...
    with open("Saves.json", 'wb') as f:
        f.write((xxhash.xxh32(pla, seed=0x6031D5ED).intdigest() ^ 0x7E7EB288).to_bytes(4, "little"))
        f.write(salt_iv)
        f.write((AES.new(PBKDF2("FH`[GBsrAd&%^as*#SDFds",
                    salt_iv, dkLen=16, count=10, hmac_hash_module=SHA1),
                    AES.MODE_CBC, salt_iv)).encrypt(pad(pla, AES.block_size)))